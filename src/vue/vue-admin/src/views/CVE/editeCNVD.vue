<template>
  <div>
    <div style="margin-bottom: 30px">编辑CNVD</div>
...
export default {
  ...
  created() {
    ...
    request.get("/cnvd/" + id).then(res => {
          console.log("Raw response data:", res.data); // Log the entire response for inspection
          this.cnvd = res.data[0]; // Check if the data is at the expected location
          console.log("User data fetched:", this.cnvd);
        })
        .catch(error => {
  ...
  methods: {
    save() {
      request.put('/cnvd/updateCNVD',this.cnvd).then(res =>{
        if(res.code == '200'){
